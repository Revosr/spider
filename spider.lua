...
local spider_Msg = database:sismember(bot_id.."List:Filter:text"..msg.chat_id_,text) 
if spider_Msg then    
...
local InfoSearch = https.request('https://bessx.tk/Code/serch.php?search='..URL.escape(Ttext)) 
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, Function_spider, nil)
return false
end
...
tdcli_function ({ID = "SearchPublicChat",username_ = username}, Function_spider, nil)
return false
end
...
if text == ("اضف مطور ثانوي") and tonumber(msg.reply_to_message_id_) ~= 0 and spiderDevSou(msg) then
function Function_spider(extra, result, success)
...
if text and text:match("^اضف مطور ثانوي @(.*)$") and spiderDevSou(msg) then
...
if text and text:match("^اضف مطور ثانوي (%d+)$") and spiderDevSou(msg) then
...
if text == ("حذف مطور ثانوي") and tonumber(msg.reply_to_message_id_) ~= 0 and spiderDevSou(msg) then
function Function_spider(extra, result, success)
...
if text and text:match("^حذف مطور ثانوي @(.*)$") and spiderDevSou(msg) then
...
if text and text:match("^حذف مطور ثانوي (%d+)$") and spiderDevSou(msg) then
...
if text == ("الثانويين") and Devspider(msg) then
...
if text == ("مسح الثانويين") and spiderDevSou(msg) then
...
if text == ("اضف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Devspider(msg) then
if AddChannel(msg.sender_user_id_) == false then
local textchuser = database:get(bot_id..'text:ch:user')
if textchuser then
send(msg.chat_id_, msg.id_,'['..textchuser..']')
...
if text == ("حذف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Devspider(msg) then
if AddChannel(msg.sender_user_id_) == false then
local textchuser = database:get(bot_id..'text:ch:user')
if textchuser then
send(msg.chat_id_, msg.id_,'['..textchuser..']')
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
local spiderrt = database:get(bot_id.."Comd:New:rt:bot:"..RTPA..msg.chat_id_)
if spiderrt == "مميز" and Addictive(msg) then
send(msg.chat_id_, msg.id_,"⋄︙العضو > ["..data.first_name_.."](t.me/"..(data.username_ or "spider_SoUrcE")..")".."\n⋄︙تم رفعه "..RTPA.." هنا\n")
...
if result.id_ then
local spiderrt = database:get(bot_id.."Comd:New:rt:bot:"..text1[2]..msg.chat_id_)
if spiderrt == "مميز" and Addictive(msg) then
send(msg.chat_id_, msg.id_,"⋄︙العضو > ["..result.title_.."](t.me/"..(text1[3] or "spider_SoUrcE")..")".."\n⋄︙تم رفعه "..text1[2].." هنا")   
database:sadd(bot_id.."Special:User"..msg.chat_id_,result.id_)  
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
usertext = '\n⋄︙> العضو > ['..data.first_name_..'](t.me/'..(data.username_ or 'spider_SoUrcE')..') ~'
...
usertext = '\n⋄︙العضو > ['..result.title_..'](t.me/'..(username or 'spider_SoUrcE')..')'
status  = '\n⋄︙تم تقيده لمدة  { '..TextEnd[2]..' '..TextEnd[3]..'}'
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
usertext = '\n⋄︙العضو > ['..data.first_name_..'](t.me/'..(data.username_ or 'spider')..') '
...
if text == ("ردود المطور") and Devspider(msg) then 
local list = database:smembers(bot_id.."List:Rd:Sudo")
text = "\n⋄︙قائمة ردود المطور \n — — — — — — — — —\n"
for k,v in pairs(list) do
...
if text == "حذف رد للكل" and Devspider(msg) then
if AddChannel(msg.sender_user_id_) == false then
local textchuser = database:get(bot_id..'text:ch:user')
if textchuser then
send(msg.chat_id_, msg.id_,'['..textchuser..']')
...
if database:get(bot_id.."Status:Bc") and not Devspider(msg) then 
...
os.execute('wget https://raw.githubusercontent.com/Revosr/spider/master/spider.lua')
os.execute('wget https://raw.githubusercontent.com/Revosr/spider/master/start.lua')
dofile('spider.lua')  
return false
end
if text == "راسلني" then
...
if text == 'السيرفر' and Devspider(msg) then 
...
tdcli_function ({ ID = "GetChannelFull", channel_id_ = msg.chat_id_:gsub("-100","")}, function(arg,data)  
if tonumber(data.member_count_) < tonumber(database:get(bot_id..'Num:Add:Bot') or 0) and not Devspider(msg) then
...
if not Devspider(msg) and not database:sismember(bot_id..'BaN:In:User',msg.sender_user_id_) and not database:get(bot_id..'Texting:In:Bv') and not database:get(bot_id.."Filter"..msg.sender_user_id_) then
...
if text =='تغيير المطور الاساسي ⋄' and spiderDevSou(msg) then
send(msg.chat_id_, msg.id_,'⋄︙ارسل ايدي المطور الاساسي الجديد')
database:set(bot_id..'Ed:DevBots',true) 
end
if text =='تغيير المطور الاساسي ⋄' and not spiderDevSou(msg) then
send(msg.chat_id_, msg.id_,'⋄︙تسرسح')
end
if text == ("مسح الثانويين ⋄") and not spiderDevSou(msg) then
send(msg.chat_id_, msg.id_,'⋄︙تسرسح')
end
if text == ("مسح الثانويين ⋄") and spiderDevSou(msg) then
database:del(bot_id.."DEV:Sudo:T")
...
if text == "حذف رساله الاشتراك ⋄ ." and Devspider(msg) then  
...
------------------------------------------------------------------------
if text then
local spider_Msg = database:sismember(bot_id.."List:Filter:text"..result.chat_id_,text) 
if spider_Msg then    
